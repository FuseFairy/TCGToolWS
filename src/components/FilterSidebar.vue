  ...
  <aside v-bind="$attrs" class="d-flex flex-column flex-shrink-0"
    ...
    <v-sheet :rounded="smAndUp ? '3md' : false"
      ...
      <div class="d-flex flex-column ga-4">
        <v-text-field label="关键字" placeholder="卡号、卡名、效果" hide-details clearable v-model="filterStore.keyword"
        ...
        <v-select label="种类"
          ...
          hide-details multiple chips clearable v-model="filterStore.selectedCardTypes" variant="outlined"></v-select>

        <v-select label="颜色"
          :items="[{ title: '黄', value: '黄色' }, { title: '绿', value: '绿色' }, { title: '红', value: '红色' }, { title: '蓝', value: '蓝色' }]"
          hide-details multiple chips clearable v-model="filterStore.selectedColors" variant="outlined"></v-select>

        <v-select label="产品" :items="filterStore.productNames" hide-details clearable
          v-model="filterStore.selectedProductName" variant="outlined"></v-select>

        <v-select label="特征" :items="filterStore.traits" hide-details multiple chips clearable
          v-model="filterStore.selectedTraits" variant="outlined"></v-select>

        <v-select label="等级" :items="['0', '1', '2', '3']" hide-details multiple chips clearable
          v-model="filterStore.selectedLevels" variant="outlined"></v-select>

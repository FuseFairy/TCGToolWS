...
export const handleSendVerificationCode = async (c) => {
  ...
  try {
    ...
    if (!password) {
      return c.json({ success: false, message: '请提供密码' }, 400)
    }
    if (password.length < 8) {
      return c.json({ success: false, message: '密码长度不能少于8位' }, 400)
    }
...
export const handleRefreshSession = async (c) => {
  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return c.json({ success: false, message: '身份验证失败' }, 401)
    }
    ...
    if (!user) {
      return c.json({ success: false, message: '使用者不存在' }, 401)
    }
    ...
    return c.json({ success: false, message: '憑證失效' }, 401)
  }
}
...
export const handleForgotPasswordRequest = async (c) => {
  ...
  try {
    ...
    if (!email) {
      return c.json({ success: false, message: '需要提供邮箱地址' }, 400)
    }
...
export const handleResetPassword = async (c) => {
  ...
  try {
    ...
    if (!token || !password || password.length < 8) {
      return c.json({ success: false, message: '需要提供有效的 Token 和至少 8 位的新密码' }, 400)
    }
    ...
    const resetRequest = await db
      ...
      .first()
    if (!resetRequest) {
      return c.json({ success: false, message: '无效的重置链接' }, 400)
    }
    ...
    if (Math.floor(Date.now() / 1000) > resetRequest.expires_at) {
      ...
      return c.json({ success: false, message: '重置链接已过期，请重新申请' }, 400)
    }
    ...
    return c.json({ success: false, message: '服务器内部错误，请稍后重试' }, 500)
  }
}
